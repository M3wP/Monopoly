...
procedure TXSIDConfigForm.ComboBox6Change(Sender: TObject);
	...
	if not FPopulating then
		begin
		FConfig.Renderer:= XSIDGlobalRenderers[ComboBox6.ItemIndex].GetName;
		DoSetupRenderParam;
...
procedure TXSIDConfigForm.FormCreate(Sender: TObject);
	...
	FConfig.Assign(XSIDGlobalConfig);
...
procedure TXSIDConfigForm.PopulateDialog;
	...
	try
		...
		for i:= 0 to XSIDGlobalRenderers.Count - 1 do
			ComboBox6.Items.Add(string(XSIDGlobalRenderers[i].GetName));

		r:= XSIDGlobalRenderers.ItemByName(FConfig.Renderer);
		i:= XSIDGlobalRenderers.IndexOf(r);
